...
const Slide: React.FC<SlideProps> = ({
  ...
  const slideRef = useRef<HTMLLIElement | null>(null); // Initialize with null since it's a DOM element ref
  const xRef = useRef<number>(0); // Initialize with 0
  const yRef = useRef<number>(0); // Initialize with 0
  const frameRef = useRef<number | undefined>(undefined); // Initialize as undefined
  ...
  const handleMouseMove = (event: React.MouseEvent<HTMLLIElement>) => {
    ...
    xRef.current = event.clientX - (rect.left + Math.floor(rect.width / 2));
    yRef.current = event.clientY - (rect.top + Math.floor(rect.height / 2));
  };
